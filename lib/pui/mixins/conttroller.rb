...
module PUI
  module Mixins
    module Controller
      ...
      # def decorator_class = "#{model.name.deconstantize}::Decorator::#{model.name.demodulize}".constantize
      # def records_view_class = "Views::#{model.name}::Records".constantize
      # def show_view_class = "Views::#{model.name}::Show".constantize
      # def index_view_class = "Views::#{model.name}::Index".constantize
      # def new_view_class = "Views::#{model.name}::Edit".constantize
      ...
      def records_view_class = "#{module_name('Views')}::Records".constantize
      def show_view_class = "#{module_name('Views')}::Show".constantize
      def index_view_class = "#{module_name('Views')}::Index".constantize
      def new_view_class = "#{module_name('Views')}::Edit".constantize
      def permitted_attributes = params.require(model.model_name.param_key).permit(permitted_attributes_keys)
